  ...
  test_that("can be used with a subset of variables (and penalty factors work)", {
    ...
    for (t in TEST.TYPES) {
      ...
      for (covar in sample(lcovar, 1)) {
        ...
        expect_warning(
          mod.bigstatsr7 <- big_spLinReg(X, y[ind], ind.train = ind,
                                         covar.train = covar[ind, ],
                                         alphas = alpha,
        ...
        expect_length(mod.bigstatsr7, 3)
        expect_equal(summary(mod.bigstatsr7)$pf.num, c(1, 2, 3))
        expect_equal(summary(mod.bigstatsr7, sort = TRUE)$pf.num, c(3, 1, 2))
